...
\begin{proof}
For a given expert $i$ let all other experts report truthfully their signals. By the previos lemma the only effect a expert has on their payment is via their effect on the chosen action  $c$. Given all other experts are truthful then by the construction of the mechanism if the $i$ expert is also truthful the mechanism will select the $c$ that maximizes $r$, thus maximizing the experts $i$ reward. Thus truthfulness is a BNE.
