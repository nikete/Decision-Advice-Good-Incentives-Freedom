...
\begin{proof}
There is two parts to the expression $r$, and $E[\hat{r}_{-i}]$. The seccond is explcitly not using any report y expert $i$, while the only effect reports can have on $r$ is via a change of chosen action. 
\end{proof}

\begin{eg}[redundant experts]
	If you have two experts and they both observe and report the same signal, they both get payoff zero. 
\end{eg}
