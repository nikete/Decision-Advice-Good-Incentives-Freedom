...
\NDP{is this notation for sensible?}
...
Thus there is a fundamental limit to the information that can be aggregated by market like mechanisms that rely on prices for information transmition (a indirect mechanism). As ilustrated above, the equilibrium reached by an indirect communication is not necesarily a direct communication equilibrium (proposition 3 in  \cite{geanakoplos1982we}). 
%At a fundamental level, if the price space is smaller than the intrinsic dimensionality 
%\NDP{this cant be totaly right, one could do some Z curve type encoding of any finite signals space into the price line}
...
An immediate corollary to a theorem X of \cite{chen2014eliciting} states that Randomized decision rules with full support are necesary for incentive compatible SVDM mechanisms.
...
\section{A Direct Mechanism for Efficient Optimal Choice Elicitation with Freedom}
...
This condition can be somewhat relaxed, at the cost of a loss of interpretation of the prices in the bidding mechanism, since the needed property it implies is that the mechanism choosing an action with higher expected reward if actually carried out leads to the expected reward conditional on the chosen action to be higher. Formally the condition that is sufficient  is:
%TODO clean this up
\[
