...
\[
  \rho_i  := \alpha \pi_i
\]
The rescaling parameter $\alpha>0$ can be chosen \emph{a priori} based on $P$ such that: $ E[\sum \rho_i] < E[r|c(s,P)] - E[r|c(P)] $. 

 $\alpha =    \frac{E[r|c(s,P)] - E[r|c(P)] )}{\sum \pi_i}  $. 

%\NDP{TODO spell out how. This seems like it should be just algebra, for exmaple if aprior I expect 10 complementary signals which combined produce a expected surplus of 2 then i want to have $g = \alpha pi$ and $\alpha < 2/10$. How can we make this procedure generic? }
...
\begin{proof}
	As they are set \emph{a priori} based on expectations only, $\alpha$ is invariant to any reports the experts make. Since the transformation is a positive rescaling, whichever report maximizes payoffs remains unchanged. Since 
\end{proof}
