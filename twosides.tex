...
\NDP{There are situations (like a expert has the signal needed to unlock for each period the meaning of the signals) where we do get exploration, so this needs to be softened....}
\begin{lem}
...
\NDP{Can we characterize when this inefficiency arises? it seems very general through clearly not always; basically the experts need to learn (their signals dont point a priri the optimal action but the mapping can be learnt from experience) and the signals are spread out between different experts (if there is a single one)}
